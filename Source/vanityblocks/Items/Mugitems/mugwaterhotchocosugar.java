	...
	public ItemStack onEaten(ItemStack par1ItemStack, World par2World,
		...
		if (!par3EntityPlayer.capabilities.isCreativeMode) {
			if (!par2World.isRemote) {
				int duration = 0;
				PotionEffect potion;
				potion = par3EntityPlayer
						.getActivePotionEffect(Potion.regeneration);
				if (potion != null)
					duration = potion.duration;
				par3EntityPlayer.addPotionEffect(new PotionEffect(
						Potion.regeneration.id, duration + 10 * 20, 0));
				...
				if (potion != null)
					duration = potion.duration;
				par3EntityPlayer.addPotionEffect(new PotionEffect(
						Potion.moveSpeed.id, duration + 15 * 20, 0));
			}
	...
	public void registerIcons(IconRegister iconRegister) {
		...
		for (int i = 0; i < this.icons.length; ++i) {
			this.icons[i] = iconRegister
					.registerIcon("vanityblocks:mugwaterhotchocosugar");
		}
	}
}
